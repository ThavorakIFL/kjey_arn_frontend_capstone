...
//                 // Update genre map for search
//                 setGenreMap(fetchedGenreMap);

//                 // Filter available genres to only show those that exist in backend
//                 const availableBackendGenres = genreOptions.filter((genre) =>
//                     fetchedGenreMap.hasOwnProperty(genre)
//                 );

//                 // If we have backend genres, use them, otherwise fall back to enum
//                 if (availableBackendGenres.length > 0) {
//                     setAvailableGenres(availableBackendGenres);
//                 } else {
//                     // Keep using your existing genreOptions as fallback
//                     setAvailableGenres(genreOptions);
//                 }
//             } catch (error) {
//                 console.error("Failed to load genres:", error);
//                 // Keep your existing genreOptions and create fallback mapping
//                 const fallbackMap: Record<string, number> = {};
//                 genreOptions.forEach((genre, index) => {
//                     fallbackMap[genre] = index + 1;
//                 });
...
//             // Convert genre names to IDs using the dynamic mapping
//             const genre_ids = selectedGenres
//                 .map((genre) => genreMap[genre])
//                 .filter((id) => id !== undefined); // Filter out any unmapped genres
...
//         // Call onSearch callback if provided (keeping original string[] format)
//         if (onSearch) {
...
const SearchAndFilterBar: React.FC<SearchAndFilterBarProps> = ({
    ...
    const [showGenreFilter, setShowGenreFilter] = useState(false);
    const [genreMap, setGenreMap] = useState<Record<string, number>>({});
    const [reverseGenreMap, setReverseGenreMap] = useState<Record<number, string>>({});
    const [isLoadingGenres, setIsLoadingGenres] = useState(false);
    const [availableGenres, setAvailableGenres] = useState<GenreType[]>(genreOptions);
    ...
    useEffect(() => {
        ...
        if (urlGenreIds && Object.keys(reverseGenreMap).length > 0) {
            const genreIds = urlGenreIds.split(",").map(id => parseInt(id.trim()));
    ...
    }, [genresLoaded, reverseGenreMap]); // Only depend on genresLoaded and reverseGenreMap
